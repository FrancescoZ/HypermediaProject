...
function init() {
  ...
  for (var i in egservices) {
    $('#services-panel').append(
      `<div class="col-xs-6 col-lg-4"><h3>${egservices[i].name}</h3><p>${egservices[i].description}</p><p><a class="btn btn-info" onClick="clickService(${egservices[i].id})" role="button">View details &raquo;</a></p></div>`)
  }
}
