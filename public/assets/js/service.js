...
function init() {
  ...
  for (var i in egdoctors) {
    if (i == 0) {
      ...
      `<div class="col-sm-6 col-md-3 featurette" style="text-align:center;"><img class="img-circle center-img elevate" src="../assets/img/doctor.png" alt="Generic placeholder image" height="110" width="110"><h4>${egdoctors[i].name}</h4><p><a class="btn btn-primary" onClick="clickDoctor(${egdoctors[i].id})" role="button">View details &raquo;</a></p></div>`)
      continue
    }
    $('#doctors-panel').append(
      `<div class="col-sm-6 col-md-3 featurette" style="text-align:center;"><img class="img-circle center-img elevate" src="../assets/img/doctor.png" alt="Generic placeholder image" height="110" width="110"><h4>${egdoctors[i].name}</h4><p><a class="btn btn-info" onClick="clickDoctor(${egdoctors[i].id})" role="button">View details &raquo;</a></p></div>`)
  }

  for (var i in eglocations) {
    $('#locations-panel').append(
      `<div class="col-sm-6 col-md-3 featurette" style="text-align:center;"><img class="img-circle center-img elevate" src="../assets/img/location.png" alt="Generic placeholder image" height="110" width="110"><h4>${eglocations[i].name}</h4><p><a class="btn btn-info" onClick="clickLocation(${eglocations[i].id})" role="button">View details &raquo;</a></p></div>`)
  }
}
